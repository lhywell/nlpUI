var host = 'http://d114.mlamp.co:8910';
//var host = 'http://192.168.24.198:8910';
//console.log(host);
define("switch", ["backbone", "bowser", "nunjucks"], function(e, t, n) {
    var i = e.Model.extend({
            initialize: function(e, t, n) {
                this.set("id", this.cid), this.set("value", e), this.set("title", t), this.set("isDefault", n === !0)
            }
        }),
        ...
        r = e.View.extend({
            ...
            render: function(e, i, a) {
                this.collection = e, e.length > 2 && this.$el.addClass("switch-" + e.length), _.each(e.models, function(e) {
                    e.set("viewname", i)
                });
                var r = n.renderString(this.tmplate, {
                    collection: e.toJSON()
                });
        ...
        s = r.extend({
            lock: function() {
                this.$("input:radio").attr("disabled", !0)
            },
            unlock: function() {
                this.$("input:radio").attr("disabled", !1)
            }
        });
    return {
        SwitchModel: i,
        ...
        r = function(a) {
            ...
            if (!(r.length > 0 && 1 == r.val())) {
                var s = a.selector || "body",
                    o = 1e3 * (a.time || 15),
                    l = e(i);
                t.msie && t.version <= 8 && l.addClass("low-bowser"), e(s).append(l), setTimeout(function() {
                    e(s).append(l), l.fadeIn(), e(window).scroll(function() {
                        n(l)
                    })
                }, o)
            }
    ...
    function n(n, s) {
        ...
        var l = s.message,
            d = s.el;
        return d = e(d ? d : "<div>" + l + "</div>"), s.hasCancel === !0 && e.extend(!0, o, {
            buttons: {
                cancel: {
                    text: "取消",
                    class: "cancel btn",
    ...
    var n = e.View.extend({
            ...
            removeEvent: function() {
                this.hasEvent = !1, $(this.enterEl).unbind("mouseenter", $.proxy(this.showEl, this)).unbind("mouseleave", $.proxy(this.hideEl, this))
            }
        }),
        i = n.extend({
            title: "查看文档",
        ...
        r = n.extend({
            ...
            initialize: function(e) {
                n.prototype.initialize.call(this, e), this.data = new a
            },
            request: function() {
            ...
            removeData: function() {
                this.data.set("api", null), this.data.set("text", null), this.data.set("actual", null), this.data.set("params", null), this.removeEvent(), this.$el.css("display", "none")
            },
            setData: function(e) {
                "object" == typeof e && (this.data.set("api", e.api), this.data.set("text", e.text), this.data.set("actual", e.actual), this.data.set("params", e.params), this.addEvent())
            }
        });
    return {
        HoverWidget: n,
    ...
    function t() {
        var t = i.offset().left,
            s = i.width();
        var div = document.documentElement.clientWidth;
        n.css("left", div-100);
    ...
    e(window).scroll(function() {
        e(this).scrollTop() ? (t(), n.fadeIn()) : n.fadeOut()
    }), n.click(function() {
        e("html, body").animate({
            scrollTop: 0
        }, 1e3)
    ...
    function basicanalysis(data) {
        var html = '';
        var html2 = '';
        var c = e('#overview-analysis');
        e.post(host + "/basicanalysis", data, function(result) {
            //console.log(result)
            var result = JSON.parse(result);
            //console.log(result['segment'])
            //console.log(result['segment'].length)
            if (result['segment'].length > 0) {
                for (var i = 0; i < result['segment'].length; i++) {
                    var name = (result['segment'][i].split(' ')[0]);
                    //console.log(name)
                    var classname = result['segment'][i].split(' ')[1];

                    html += '<dd class=' + classname + '>' + name + '</dd>'
                }
                e('.words', c).html(html);
            }
        });
    ...
    //实体识别
    function namerecognization(data){
        var html2 = '';
        var s = e('#overview-ner');
        e.post(host + "/namerecognization", data, function(result) {
            //console.log(result)
            var result = JSON.parse(result);
            //console.log(result['segment'])
            //console.log(result['segment'].length)
            if (result['namerecognization'].length > 0) {
                for (var i = 0; i < result['namerecognization'].length; i++) {
                    var name = (result['namerecognization'][i].split(' ')[0]);
                    var entity = result['namerecognization'][i].split(' ')[1];

                    html2 += '<dd class=' + entity + '>' + name + '</dd>'
                }
                e('.words', s).html(html2);
            }
        });
    }
    //依存文法
    function dependency(data) {
        var html = '';
        var f = e('#overview-depend');
        e.post(host + "/dependency", data, function(result) {
            var result = JSON.parse(result);
            //console.log(result.dependency)
            e('.depend-tree', f).html('<img src="data:image/jpg;base64,' + result.dependency + '" />');
        });
    }
    //文本分类
    function textclass(data) {
        var html = '';
        var f = e('#overview-info');
        e.post(host + "/textclass", data, function(result) {
            //console.log(result)
            var result = JSON.parse(result);
            e('.info-parent-containers', f).html('<span>一级分类：' + result.textClass.parentClass + '</span>');
            e('.info-child-containers', f).html('<span>二级分类：' + result.textClass.childClass + '</span>');
        });
    }
    //关键词提取
    function keywords(data) {
        var html = '';
        var f = e('#overview-key');
        e.post(host + "/keywords", data, function(result) {
            var result = JSON.parse(result);
            //console.log(result)
            var key = result.keywords,
                html = ''
            for (var i = 0; i < key.length; i++) {
                var td_key = key[i].split(' ')[0];
                var td_value = key[i].split(' ')[1];
                html += '<tr><td>' + td_key + '</td><td>' + td_value + '</td></tr>'
            }
            e('.result', f).html('<div class="col-small-0"><table><thead><tr><td>名称</td><td>权重</td></tr></thead><tbody>' + html + '</tbody></table></div>');
        });
    }
    //情感分析
    function sentiment(data) {
        var html = '';
        var f = e('#overview-key');
        e.post(host + "/sentiment", data, function(result) {
            var result = JSON.parse(result);
            //console.log(result.sentiment)
            var negative = result.sentiment.negative;
            var positive = result.sentiment.positive;
            var neutral = result.sentiment.neutral;
            //console.log(positive)
            Highcharts.getOptions().plotOptions.pie.colors = (function() {
                var colors = ["rgb(77,175,124)", "rgb(229,97,59)", "rgb(119,190,255)"];
                //console.log(colors)
                return colors;
            }());
            ...
            Highcharts.chart('highcharts-0', {
                ...
                series: [{
                    name: '百分比',
                    data: [
                        { name: '正面指数', y: Number(positive) },
                        { name: '负面指数', y: Number(negative) },
                        { name: '中性指数', y: Number(neutral) },
                    ]
                }]
            });
        });
    ...
    function summary(data) {
        var html = '';
        var f = e('#overview-summary');
        e.post(host + "/summary", data, function(result) {
            var result = JSON.parse(result);
            //console.log(result)
            e('.result', f).html(result.textSummary);
        });
    }
    //语义联想
    function association(data) {
        var html = '';
        var f = e('#overview-semantic');
        e.post(host + "/association", data, function(result) {
            var result = JSON.parse(result);
            //console.log(result.association)
            var association = result.association;
            for (var i = 0; i < association.length; i++) {
                var words = '';
                for (var y = 0; y < association[i].associationalWords.length; y++) {
                    var name = association[i].associationalWords[y].split(' ')[0];
                    var num = association[i].associationalWords[y].split(' ')[1];
                    words += '<tr><td>' + name + '</td><td>' + num + '</td></tr>';
                }
                var column = '';
                if (i % 2 == 0) {
                    column = 0;
                ...
                html += '<div class="col-' + column + '"><div class="suggest-word">关键词：' + association[i].keyword + '</div><table><thead><tr><td>名称</td><td>相关性</td></tr></thead><tbody>' + words + '</tbody></table></div>'
            }
            e('.result', f).empty().html(html);
        });
    }

    function handleData(data) {
        basicanalysis(data);
        namerecognization(data);
        dependency(data);
        textclass(data);
        keywords(data);
        sentiment(data);
    ...
    e('#btn-reset').click(function() {
        e("#txt-analysis").val('');
    })

    var saveArray = [];
    //保存到左侧
    function saveLeft(data) {
        //console.log(data)
        saveArray.unshift(data);

        var html = '';
        for (var i = 0; i < saveArray.length; i++) {
            html += '<div class="item">' + saveArray[i] + '</div>';
        }
        e("#insert-text").html(html);
        e("#insert-text .item").eq(0).addClass('active');
    }

    //事件委托
    e("#insert-text ").delegate('.item', 'click', function(event, index) {
        //console.log(1)
        e(this).addClass('active').siblings().removeClass('active');
        var data = e(this).text();
        handleData(data);
    })

    //事件
    e('#btn-analysis').click(function() {
        var txt = s();
        var data = p(txt);
        ...
        if (data != '国家主席习近平指出，非洲联盟是非洲联合自强的旗帜。非盟积极推进非洲一体化进程，推动非洲国家在重大国际和地区问题上用一个声音说话，取得显著成果。衷心祝愿非盟引领非洲国家在和平与发展的事业中不断取得新的更大成就。习近平强调，中非合作论坛约翰内斯堡峰会成果落实取得重要早期收获，助力非洲经济社会发展。中方将继续秉持真实亲诚对非政策理念和正确义利观，积极推动中非“十大合作计划”和“一带一路”建设同非盟《2063年议程》对接，推动中非全面战略合作伙伴关系深入发展，更好造福中非人民。') {
            e("#txt-analysis").val('');
            saveLeft(data);
            handleData(data);
        }else{
    ...
    //初始化
    function init() {
        var data = '国家主席习近平指出，非洲联盟是非洲联合自强的旗帜。非盟积极推进非洲一体化进程，推动非洲国家在重大国际和地区问题上用一个声音说话，取得显著成果。衷心祝愿非盟引领非洲国家在和平与发展的事业中不断取得新的更大成就。习近平强调，中非合作论坛约翰内斯堡峰会成果落实取得重要早期收获，助力非洲经济社会发展。中方将继续秉持真实亲诚对非政策理念和正确义利观，积极推动中非“十大合作计划”和“一带一路”建设同非盟《2063年议程》对接，推动中非全面战略合作伙伴关系深入发展，更好造福中非人民。';
        e("#txt-analysis").val(data);
        saveLeft(data);
        handleData(data);
    }
    init();
